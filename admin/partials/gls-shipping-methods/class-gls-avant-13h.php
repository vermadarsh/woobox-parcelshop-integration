 ...
 * GLS_Avant_13H class.
 */
class GLS_Avant_13H extends WC_Shipping_Method {
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->id                 = 'woopi-gls-avant-13h';
		$this->instance_id        = absint( $instance_id );
		$this->method_title       = __( 'GLS Avant 13H', 'woopi' );
		$this->method_description = __( 'Livraison Express en 24H en France métropolitaine, remise en mains propres le lendemain avant 13H.', 'woopi' );
		$this->supports           = array(
			'shipping-zones',
			'instance-settings',
	...
	public function calculate_shipping( $package = array() ) {
		...
		if ( ! empty( $package['contents'] ) ) {
			...
			$destination_postcode = ( ! empty( $package['destination']['postcode'] ) ) ? $package['destination']['postcode'] : false;
			$destination_country  = ( ! empty( $package['destination']['country'] ) ) ? $package['destination']['country'] : false;

			if ( false !== $destination_postcode && false !== $destination_country ) {
				$cost += woopi_get_extra_shipping_cost_gls_shipping_method( $destination_country, $destination_postcode, $this->instance_settings );
			}
	...
	public function init_form_fields() {
		...
		$settings = array(
			'title'                            => array(
				...
				'default'     => __( 'GLS Avant 13H', 'woopi' ),
				'desc_tip'    => true,
			),
			'tax_status'                       => array(
