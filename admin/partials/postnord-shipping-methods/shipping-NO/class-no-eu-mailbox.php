 ...
 * NO_EU_Mailbox class.
 */
class NO_EU_Mailbox extends WC_Shipping_Method {
	...
	public function __construct( $instance_id = 0 ) {
		...
		$this->id                 = 'woopi-pn-no-eu-mailbox';
		$this->instance_id        = absint( $instance_id );
		$this->method_title       = __( 'NO EU Mailbox', 'woopi' );
		$this->method_description = __( 'Shipping method dealing - Norway EU Mailbox.', 'woopi' );
		$this->supports           = array(
			'shipping-zones',
			'instance-settings',
	...
	public function calculate_shipping( $package = array() ) {
		...
		if ( ! empty( $package['contents'] ) ) {
			...
			if ( ! empty( $this->woopi_custom_rates ) ) {
				...
				if ( ( $total_weight >= $weight_minimum && $total_weight <= $weight_maximum ) && ( $cart_total >= $total_from && $cart_total <= $total_to ) ) {
	...
	public function init_form_fields() {
		$settings = array(
			'title'      => array(
				...
				'default'     => __( 'NO EU Mailbox', 'woopi' ),
				'desc_tip'    => true,
			),
			'tax_status' => array(
